...
object AppConfig {
    ...
    fun load(application: Application): WeatherConfig {
        ...
        val weatherProviderConfig = WeatherProviderConfig(
            ...
            fallbackEnabled = config.propertyOrNull("weather.provider.fallbackEnabled")?.getString()?.toBoolean() ?: false,
        ...
        val cacheConfig = CacheConfig(
            weather = CacheTypeConfig(
                ...
                durationMinutes = config.propertyOrNull("cache.weather.durationMinutes")?.getString()?.toIntOrNull() ?: 15
            ),
            forecast = CacheTypeConfig(
                provider = config.propertyOrNull("cache.forecast.provider")?.getString() ?: "CAFFEINE",
                durationMinutes = config.propertyOrNull("cache.forecast.durationMinutes")?.getString()?.toIntOrNull() ?: 60
            ),
            location = CacheTypeConfig(
                provider = config.propertyOrNull("cache.location.provider")?.getString() ?: "CAFFEINE",
                durationMinutes = config.propertyOrNull("cache.location.durationMinutes")?.getString()?.toIntOrNull() ?: 1440
            ),
        ...
        logger.debug("Rate limit algorithm: {} - {} requests per {} days", 
            rateLimitConfig.algorithm, rateLimitConfig.maxRequestsPerDay, rateLimitConfig.windowSizeDays)
        logger.debug("Cache providers - Weather: {}, Forecast: {}, Location: {}", 
